                    ...
                    <div class="col-md-8">
                        <form action="{{route('roles.update', ['id' => $role->id])}}" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control @error('name') is-invalid @enderror " placeholder="Nhập tên role" name="name" value="{{$role->name}}">
                                @error('name')
                            ...
                            <div class="form-group">
                                <label>Mô tả</label>
                                <input type="text" class="form-control @error('display_name') is-invalid @enderror " placeholder="Nhập mô tả role" name="display_name" value="{{$role->display_name}}">
                                @error('display_name')
                                <div class="alert alert-danger p-2 mt-2">{{ $message }}</div>
                                @enderror
                            </div>
                                                ...
                                                @foreach($permission->permissionChild as $permissionChildrent)
                                                    <div class="col-3">
                                                        <div class="card-body text-primary">
                                                            <div class="custom-control ">
                                                                <div class="form-check">
                                                                    <input class="form-check-input check-box-permission-child" type="checkbox" {{in_array($permissionChildrent->id, $permissionRole) ? 'checked' : ''}} name="permission_id[]" value="{{$permissionChildrent->id}}">
                            ...
                            <button type="submit" class="btn btn-primary float-right" name="btn-update-role">Chỉnh sửa</button>
                        </form>
                    </div>
